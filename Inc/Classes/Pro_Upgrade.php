...
use JLTADMINBAR\Inc\Classes\Notifications\Base\Date;
...
class Pro_Upgrade {
	...
	public function __construct() {
		$this->slug = Helper::jltadminbar_slug_cleanup();
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltadminbar_dashboard_widget',                          // Widget slug.
		...
		$example_widget_backup = array( 'jltadminbar_dashboard_widget' => $default_dashboard['jltadminbar_dashboard_widget'] );
		unset( $default_dashboard['jltadminbar_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="jltadminbar-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="jltadminbar-dashboard-promo" style="--jltadminbar-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				...
				<a class="jltadminbar-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'admin-bar' );
		...
		<div class="jltadminbar-dashboard_footer">
			<ul>
				<li class="jltadminbar-overview__blog">
		...
		<style>
			...
			.jltadminbar-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltadminbar-rss-widget .jltadminbar-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltadminbar-rss-widget .jltadminbar-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltadminbar-rss-widget .jltadminbar-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltadminbar-rss-widget .jltadminbar-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltadminbar-rss-widget .jltadminbar-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jltadminbar_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltadminbar_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltadminbar_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTADMINBAR_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jltadminbar_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltadminbar_sheet_promo_data', $remote_data );
			update_option( 'jltadminbar_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltadminbar_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltadminbar_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="jltadminbar-popup jltadminbar-upgrade-popup" id="jltadminbar-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="jltadminbar-popup-overlay"></div>

			<div class="jltadminbar-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltadminbar-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">

				<!-- close  -->
				<div class="jltadminbar-popup-modal-close popup-dismiss">×</div>

				<!-- content section  -->
				<div class="jltadminbar-popup-modal-footer">

					<!-- countdown  -->
					<div class="jltadminbar-popup-countdown" style="display: none;">
						<span class="jltadminbar-popup-countdown-text">Deal Ends In</span>
						<div class="jltadminbar-popup-countdown-time">
							<div>
					...
					<!-- button  -->
					<a class="jltadminbar-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltadminbar-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltadminbar-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltadminbar-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltadminbar-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltadminbar-popup-countdown").hide(0);
				}
