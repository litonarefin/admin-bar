(function ($)
{
    ...
    jQuery('#ab-form').ajaxForm({
        ...
        data: {
            action: 'update_form',
            security: JLTADMINBARCORE.admin_bar_security_nonce,
        },
        success: function ()
        {
    ...
    // Notice Hide
    $('body').on('click', '.jltadminbar-upgrade-popup .popup-dismiss', function (evt)
    {
        evt.preventDefault();
        $(this).closest('.jltadminbar-upgrade-popup').fadeOut(200);
    });
    ...
    {
        evt.preventDefault();
        $('.jltadminbar-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltadminbar-search-plugins #search-plugins').on('keyup', function ()
    {
        var value = $(this).val();
...
function activatePlugin(element, file)
{
    ...
    jQuery.ajax({
        url: JLTADMINBARCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltadminbar_recommended_activate_plugin",
            file: file,
            nonce: JLTADMINBARCORE.recommended_nonce
        },
...
function installPlugin(element, plugin)
{
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltadminbar_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTADMINBARCORE.recommended_nonce
        },
